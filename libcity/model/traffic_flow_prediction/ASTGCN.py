...
class ASTGCNBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # (B, N, F_out, T) -> (B, F_out, N, T)

        # residual shortcut
        x_residual = self.residual_conv(x.permute(0, 2, 1, 3))
        # (B, N, F_in, T) -> (B, F_in, N, T)
...
class ASTGCN(AbstractTrafficStateModel):
    ...
    def extract_feature(self, batch, train=True, lap_mx=None):
        x = batch['X'].to(self.device)  # (B, T, N_nodes, F_in)
        # output = self.ASTGCN_submodule(x)
        output = self.ASTGCN_submodule.extract_feature(x)
        return output  # (B, T', N_nodes, F_out)

    def feature_to_pred(self, output):
        output = self.ASTGCN_submodule.feature_to_pred(output)
        return output