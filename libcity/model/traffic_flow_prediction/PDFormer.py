...
class PDFormer(AbstractTrafficStateModel):
    ...
    def extract_feature(self, batch, train=True, lap_mx=None):
        ...
        if train:
            ...
            zero = torch.zeros_like(random).to(self.device)
            one = torch.ones_like(random).to(self.device)
            random = torch.where(random > 0.75, one, zero)
            x = x * random
        T =  x.shape[1]
        x_pattern_list = []
        for i in range(self.s_attn_size):
